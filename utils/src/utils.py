...
def ppt_to_images(file: str, output_dir: str, warning: bool = False, dpi=72, output_type='png'):
    ...
    #  指定 soffice 路径
    SOFFICE_PATH = "/home/ubuntu/yifan/eeg2fmri/Paper2PPTX/LibreOffice_25.2.4.3_Linux_x86-64_deb/libreoffice-extracted/opt/libreoffice25.2/program/soffice"
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # ✅ 加入环境变量确保 subprocess 能识别
        env = os.environ.copy()
        env["PATH"] = os.path.dirname(SOFFICE_PATH) + ":" + env.get("PATH", "")


        # subprocess.run(command_list, check=True, stdout=subprocess.DEVNULL)
        subprocess.run(command_list, check=True, stdout=subprocess.DEVNULL, env=env)
