...
def get_poster_text(paper_path):
    markdown_clean_pattern = re.compile(r"<!--[\s\S]*?-->")
    converter = DocumentConverter()
    raw_result = converter.convert(paper_path)
    ...
    if len(text_content) < 500:
        ...
        text_content, rendered = parse_pdf(paper_path, model_lst=parser_model, save_file=False)
    return text_content
...
def gen_eval_markdown(paper_name, poster_method, paper_path, figure_count_only=False):
    model_name="openai/clip-vit-base-patch32"
    model_name = "BAAI/AltCLIP"
    model = AltCLIPModel.from_pretrained(model_name).to('cuda')
    processor = AltCLIPProcessor.from_pretrained(model_name)
    ...
    if poster_method != 'paper':
        ...
        png_to_pdf(paper_path, unique_pdf)
        paper_path = unique_pdf
    ...
    poster_text = get_poster_text(paper_path)
