...
class VLLMModel(BaseModelBackend):
    ...
    def __init__(
        ...
        if not self._url:
            ...
            if "llava" in self.model_type.lower():
                self._url = "http://localhost:8001/v1"
        ...
        try:
            subprocess.Popen(
                [
                    ...
                    "--port", "8001",
                    "--served-model-name", self.model_type,
                    "--max-model-len", "4096"
    ...
    def run(
        ...
        print("[DEBUG] config_dict =", self.model_config_dict)
        print(f"[DEBUG] model = {self.model_type}, base_url = {self._url}")

        print(f"[DEBUG] calling vLLM with model_type = {self.model_type}")
