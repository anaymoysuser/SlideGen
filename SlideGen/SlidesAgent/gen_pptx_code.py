...
def generate_multi_slide_ppt(
    ...
    for idx, section in enumerate(content):
        ...
        # 从 layout 中找出当前章节的 text box
        text_box_data = text_arrangement[idx]
        left = Inches(text_box_data['x'])
        top = Inches(text_box_data['y'])
        width = Inches(text_box_data['width'])
        height = Inches(text_box_data['height'])
        ...
        body_shape = slide.shapes.add_textbox(left, body_top, width, height - Inches(0.8))
        body_frame = body_shape.text_frame

        for bullet in section['textbox1']:
        ...
        # 可选 textbox2
        if 'textbox2' in section:
            for bullet in section['textbox2']:
                p = body_frame.add_paragraph()
                p.text = bullet
                p.level = 0
                p.font.size = Pt(20)
        ...
        for fig in figure_arrangement:
            if fig['panel_id'] == idx:
                fig_left = Inches(fig['x'])
                fig_top = Inches(fig['y'])
                fig_width = Inches(fig['width'])
                fig_height = Inches(fig['height'])
                fig_path = fig['figure_path']

                if os.path.exists(fig_path):
                    slide.shapes.add_picture(fig_path, fig_left, fig_top, fig_width, fig_height)
